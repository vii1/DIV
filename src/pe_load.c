...
int dump_section( const struct _IMAGE_SECTION_HEADER* h, const char* ext );
#endif
...
#ifdef PEDEBUG
	printf("WARNING! Virtual Address not found: %lx\n", va);
#endif
	return NULL;
}
...
#ifdef PEDEBUG
	if( verbosity >= 1 )
		printf( "RELOCATING: Page rva %ld, block size %ld\n", br->VirtualAddress, br->SizeOfBlock );
#endif
...
PE* PE_ReadFP( FILE* in )
{
	...
	for( t = 0; t < p->nh.FileHeader.NumberOfSections; t++ ) {
		if( p->sh[t].Characteristics & IMAGE_SCN_MEM_DISCARDABLE ) {
...
#ifdef PEDEBUG
	if( verbosity >= 1 ) printf( "Seek Function Name %s\n", funcname );
#endif
...
#ifdef PEDEBUG
				if( verbosity >= 1 ) printf( "    Found function Name %s\n", name );
#endif
