...
static const char* PE_GetSectionOfVirtualAddress( PE* p, ULONG va )
{
	int					  t;
	IMAGE_SECTION_HEADER* s;

	for( t = 0, s = p->sh; t < p->nh.FileHeader.NumberOfSections; t++, s++ ) {
		if( va >= s->VirtualAddress && va < ( s->VirtualAddress + s->SizeOfRawData ) ) {
			return ( s->Name );
		}
	}
	return NULL;
...
*/
{
	...
	for( t = 0, s = p->sh; t < p->nh.FileHeader.NumberOfSections; t++, s++ ) {
		if( va >= s->VirtualAddress && va < ( s->VirtualAddress + s->SizeOfRawData ) ) {
			return ( s->Misc.PhysicalAddress + va - s->VirtualAddress );
		}
	}
	printf( "DANGER! Linear Address not found: %lx\n", va );
	return NULL;
}
...
int dump_section( const IMAGE_SECTION_HEADER* h, const char* ext )
{
	...
	fwrite( h->Misc.PhysicalAddress, h->SizeOfRawData, 1, f );
	fclose( f );
...
static int PE_LoadSection( FILE* in, PE* p, IMAGE_SECTION_HEADER* h )
{
	...
	if( h->SizeOfRawData == 0 ) {
		h->Misc.PhysicalAddress = NULL;
		return 1;
	}

	// allocate enough memory to hold section
	virtualSize = h->Misc.VirtualSize;
	rawSize = h->SizeOfRawData;
	if( virtualSize > rawSize ) {
		h->SizeOfRawData = virtualSize;
	}
	// make sure the size is aligned
	alignOffset = h->SizeOfRawData % p->nh.OptionalHeader.SectionAlignment;
	if( alignOffset > 0) {
		h->SizeOfRawData += p->nh.OptionalHeader.SectionAlignment - alignOffset;
	}
	if( ( h->Misc.PhysicalAddress = malloc( h->SizeOfRawData ) ) == 0 ) {
	...
	if( virtualSize > rawSize ) {
		memset( h->Misc.PhysicalAddress + rawSize, 0, virtualSize - rawSize );
	}
	...
	fread( h->Misc.PhysicalAddress, rawSize, 1, in );

	if( !dump_files ) return 1;
	return dump_section( h, "001" );
}
...
*/
{
	...
	if( verbosity >= 1 ) printf( "RELOCATING: Page rva %ld, block size %ld\n", br->VirtualAddress, br->SizeOfBlock );
	...
	if( verbosity >= 1 ) printf( "    %d relocs at %p\n", numrelocs, reloc );

	for( t = 0; t < numrelocs; t++ ) {
		if( verbosity >= 2 ) printf( "Type %d, Offset %u\n", reloc[t] >> 12, reloc[t] & 0xfff );
		...
		switch( reloc[t] >> 12 ) {
			...
			case IMAGE_REL_BASED_HIGHLOW:
				...
				// find the physical address it belongs to:
				relocval = (ULONG)PE_GetLinearAddress( p, relocval );
				if(relocval == 0) printf("NULL at: %lx\n", relocadr);
				// write back that value:
				*( (ULONG*)relocadr ) = relocval;
				break;

			default: dll_error = "Unsupported relocation type"; return 0;
		}
	}
	return 1;
}
...
static int PE_Relocate( PE* p )
{
	...
	if( ( dd = PE_ImageDirectory( p, IMAGE_DIRECTORY_ENTRY_BASERELOC ) ) == NULL || dd->Size == 0 ||
	  ( br = PE_GetLinearAddress( p, dd->VirtualAddress ) ) == NULL )
		return 1;

	memcpy( name, PE_GetSectionOfVirtualAddress( p, dd->VirtualAddress ), 8 );
	name[8] = 0;

	if( verbosity >= 1 ) printf( "Found relocation data in section %s\n", name );
...
PE* PE_ReadFP( FILE* in )
{
	...
	if( verbosity >= 1 ) printf( "NT header offset at 0x%lx\n", dh.e_lfanew );
	...
	if( verbosity >= 1 ) printf( "Number of sections: %d\n", p->nh.FileHeader.NumberOfSections );
	...
	for( t = 0; t < p->nh.FileHeader.NumberOfSections; t++ ) {
		...
		if( verbosity >= 1 )
			printf( "Section: '%s'\n"
			  ...
			  p->sh[t].Name, p->sh[t].Characteristics, p->sh[t].Misc.VirtualSize, p->sh[t].VirtualAddress,
			  p->sh[t].SizeOfRawData, p->sh[t].PointerToRawData );

		if( !PE_LoadSection( in, p, &p->sh[t] ) ) {
			PE_Free( p );
			return NULL;
		}
		if( verbosity >= 1 ) printf( "    Physical address:    %p\n", p->sh[t].Misc.PhysicalAddress );
	}
...
void PE_Free( PE* p )
{
	...
	if( p->sh != NULL ) {
		for( t = 0; t < p->nh.FileHeader.NumberOfSections; t++ ) {
			...
			if( p->sh[t].Misc.PhysicalAddress != NULL ) {
				free( p->sh[t].Misc.PhysicalAddress );
			}
		}
		free( p->sh );
	}
...
void* PE_ImportFnc( PE* p, char* funcname )
{
	...
	for( t = 0; t < e->NumberOfNames; t++ ) {
		...
		if( !strcmp( funcname, cwork ) ) {
			ULONG rva = rvatable[ordtable[t]];
			void* la = PE_GetLinearAddress( p, rva );
