...
*
* $Date:   02 May 1996 03:17:06  $ $Author:   KendallB  $
*
****************************************************************************/
...
#ifndef	REALMODE
PUBLIC	short		_ASMAPI VBE_MMIOSel = 0;/* Selector to MMIO registers*/
...
****************************************************************************/
{
	if ((VESABuf_len = len) < 512) {
#ifdef	__WINDOWS__
	MessageBox(NULL,
		"Real mode block must be at least 512 bytes in length!",
		"VESAVBE.C Fatal Error!", MB_ICONEXCLAMATION);
#else
		printf("Real mode block must be at least 512 bytes in length!\n");
#endif
		exit(1);
		}
	VESABuf_sel = sel;
