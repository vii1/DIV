...
PRIVATE void czShadePoly(int count,fxpointcz_t *lArray,fxpointcz_t *rArray,
	fix32_t xOffset,fix32_t yOffset,zfix32_t zOffset)
/****************************************************************************
*
* Function:		czShadePoly
...
****************************************************************************/
{
	...
	if (DC.a.writeMode != MGL_REPLACE_MODE)
		DC.doClip = true;
	trap = DC.doClip ? __EMU__czTrap : DC.r.z.czTrap;
...
#define	V(v)	((fxpointc_t *)(v))
#define	VZ(v)	((fxpointz_t *)(&(v)->p))

	/* Setup for rendering the first edge in left list */
	while (_MGL_cComputeSlope(V(lArray),V(lArray+1),&DC.tr.slope1,&DC.tr.cSlope1) <= 0) {
		lArray++;
		if ((--count) == 0)
	...
	while (_MGL_cComputeSlope(V(rArray),V(rArray+1),&DC.tr.slope2,&DC.tr.cSlope2) <= 0)
		rArray++;
	...
	while (1) {
		...
		if (lArray[1].p.y == endy) {/* Has left edge ended?				*/
			lArray++;
			while ((count = _MGL_cComputeSlope(V(lArray),V(lArray+1),&DC.tr.slope1,&DC.tr.cSlope1)) <= 0) {
				lArray++;
				if (count < 0)
		...
		if (rArray[1].p.y == endy) {/* Has right edge ended?			*/
			rArray++;
			while ((count = _MGL_cComputeSlope(V(rArray),V(rArray+1),&DC.tr.slope2,&DC.tr.cSlope2)) <= 0) {
				rArray++;
				if (count < 0)
...
void _ASMAPI __EMU__czTri(fxpointcz_t *v1,fxpointcz_t *v2,fxpointcz_t *v3,
	fix32_t xOffset,fix32_t yOffset,zfix32_t zOffset)
/****************************************************************************
*
* Function:		__EMU__czTri
...
****************************************************************************/
{
	fxpointcz_t	*t,*lArray,*rArray;

	/* Get memory in which to store left and right edge arrays			*/
	lArray = _MGL_buf;
	rArray = (fxpointcz_t*)((uchar*)_MGL_buf + 3 * sizeof(fxpointcz_t));
	...
	czShadePoly(3,lArray,rArray,xOffset,yOffset,zOffset);
}

void _ASMAPI __EMU__czQuad(fxpointcz_t *v1,fxpointcz_t *v2,fxpointcz_t *v3,
	fxpointcz_t *v4,fix32_t xOffset,fix32_t yOffset,zfix32_t zOffset)
/****************************************************************************
*
* Function:		__EMU__czQuad
* Parameters:	v1,v2,v3	- Vertices in the triangle
*				xOffset		- Offset of x coordinates
*				yOffset		- Offset of y coordinates
*
...
****************************************************************************/
{
	fxpointcz_t	*t,*lArray,*rArray,*vt1,*vt2,*vt3;

	/* Get memory in which to store left and right edge arrays			*/
	lArray = _MGL_buf;
	rArray = (fxpointcz_t*)((uchar*)_MGL_buf + 4 * sizeof(fxpointcz_t));
