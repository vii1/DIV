...
*
* Description:	Main 8 bit accelerated device driver file. This file
...
#include "mgl.h"
#pragma hdrstop
#include "drivers\accel\accel8.h"

#if	!defined(__16BIT__) || defined(TESTING)

/*---------------------------- Driver Tables -----------------------------*/

drivertype	_VARAPI ACCEL8_driver = {
	IDENT_FLAG,
	MGL_ACCEL8NAME,
	MGL_ACCEL8NAME,
...
ibool _ASMAPI ACCEL8_detect(int id,int force,int *driver,int *mode,
	modetab modes)
/****************************************************************************
*
* Function:		ACCEL8_detect
...
****************************************************************************/
{
	if (force >= grSVGA && ACCEL_detect(driver,&ACCEL8_driver.driverId)) {
		/* Fill in information here for the available video modes by
		 * querying the ACCEL.DRV for the information.
		 */
		ACCEL_useMode(modes,grSVGA_320x200x256,id,&ACCEL8_modes[0]);
		ACCEL_useMode(modes,grSVGA_320x240x256,id,&ACCEL8_modes[1]);
		ACCEL_useMode(modes,grSVGA_320x400x256,id,&ACCEL8_modes[2]);
		ACCEL_useMode(modes,grSVGA_320x480x256,id,&ACCEL8_modes[3]);
		ACCEL_useMode(modes,grSVGA_400x300x256,id,&ACCEL8_modes[4]);
		ACCEL_useMode(modes,grSVGA_512x384x256,id,&ACCEL8_modes[5]);
		ACCEL_useMode(modes,grSVGA_640x350x256,id,&ACCEL8_modes[6]);
		ACCEL_useMode(modes,grSVGA_640x400x256,id,&ACCEL8_modes[7]);
		ACCEL_useMode(modes,grSVGA_640x480x256,id,&ACCEL8_modes[8]);
		ACCEL_useMode(modes,grSVGA_800x600x256,id,&ACCEL8_modes[9]);
		ACCEL_useMode(modes,grSVGA_1024x768x256,id,&ACCEL8_modes[10]);
		ACCEL_useMode(modes,grSVGA_1152x864x256,id,&ACCEL8_modes[11]);
		ACCEL_useMode(modes,grSVGA_1280x960x256,id,&ACCEL8_modes[12]);
		ACCEL_useMode(modes,grSVGA_1280x1024x256,id,&ACCEL8_modes[13]);
		ACCEL_useMode(modes,grSVGA_1600x1200x256,id,&ACCEL8_modes[14]);
		return true;
		}
	mode = mode;
	return false;
...
PUBLIC void ACCEL8_initInternal(MGLDC *dc)
/****************************************************************************
*
* Function:		ACCEL8_initInternal
* Parameters:	dc	- Device context.
...
ibool _ASMAPI ACCEL8_initDriver(MGLDC *dc,int driverId,int modeId,ulong hwnd,
	int virtualX,int virtualY,int numBuffers,ibool stereo,int refreshRate)
/****************************************************************************
*
* Function:		ACCEL8_initDriver
* Parameters:	dc	- Device context.
* Returns:		True if the device was correctly initialised.
*
...
****************************************************************************/
{
	...
	dc->mi = ACCEL8_modes[modeId - grSVGA_320x200x256];
	if (g_state == NULL) {
