...
#include "mgl.h"
#pragma hdrstop
#include "drivers\accel\accel32.h"

#if	!defined(__16BIT__) || defined(TESTING)

/*---------------------------- Driver Tables -----------------------------*/

drivertype	_VARAPI ACCEL32_driver = {
	IDENT_FLAG,
	MGL_ACCEL32NAME,
	MGL_ACCEL32NAME,
...
ibool _ASMAPI ACCEL32_detect(int id,int force,int *driver,int *mode,
	modetab modes)
/****************************************************************************
*
* Function:		ACCEL32_detect
...
****************************************************************************/
{
	if (force >= grSVGA && ACCEL_detect(driver,&ACCEL32_driver.driverId)) {
		/* Fill in information here for the available video modes by
		 * querying the ACCEL.DRV for the information.
		 */
		ACCEL_useMode(modes,grSVGA_320x200x4G,id,&ACCEL32_modes[0]);
		ACCEL_useMode(modes,grSVGA_320x240x4G,id,&ACCEL32_modes[1]);
		ACCEL_useMode(modes,grSVGA_320x400x4G,id,&ACCEL32_modes[2]);
		ACCEL_useMode(modes,grSVGA_320x480x4G,id,&ACCEL32_modes[3]);
		ACCEL_useMode(modes,grSVGA_400x300x4G,id,&ACCEL32_modes[4]);
		ACCEL_useMode(modes,grSVGA_512x384x4G,id,&ACCEL32_modes[5]);
		ACCEL_useMode(modes,grSVGA_640x350x4G,id,&ACCEL32_modes[6]);
		ACCEL_useMode(modes,grSVGA_640x400x4G,id,&ACCEL32_modes[7]);
		ACCEL_useMode(modes,grSVGA_640x480x4G,id,&ACCEL32_modes[8]);
		ACCEL_useMode(modes,grSVGA_800x600x4G,id,&ACCEL32_modes[9]);
		ACCEL_useMode(modes,grSVGA_1024x768x4G,id,&ACCEL32_modes[10]);
		ACCEL_useMode(modes,grSVGA_1152x864x4G,id,&ACCEL32_modes[11]);
		ACCEL_useMode(modes,grSVGA_1280x960x4G,id,&ACCEL32_modes[12]);
		ACCEL_useMode(modes,grSVGA_1280x1024x4G,id,&ACCEL32_modes[13]);
		ACCEL_useMode(modes,grSVGA_1600x1200x4G,id,&ACCEL32_modes[14]);
		return true;
		}
	mode = mode;
	return false;
...
PUBLIC void ACCEL32_initInternal(MGLDC *dc)
/****************************************************************************
*
* Function:		ACCEL32_initInternal
* Parameters:	dc	- Device context.
...
ibool _ASMAPI ACCEL32_initDriver(MGLDC *dc,int driverId,int modeId,ulong hwnd,
	int virtualX,int virtualY,int numBuffers,ibool stereo,int refreshRate)
/****************************************************************************
*
* Function:		ACCEL32_initDriver
* Parameters:	dc	- Device context.
* Returns:		True if the device was correctly initialised.
*
...
****************************************************************************/
{
	...
	dc->mi = ACCEL32_modes[modeId - grSVGA_320x200x4G];
	if (g_state == NULL) {
