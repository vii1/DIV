...
drivertype	_VARAPI PACKED16_driver = {
	IDENT_FLAG,
	MGL_PACKED16NAME,
	MGL_PACKED16NAME,
...
****************************************************************************/
{
	 ...
	 * when we do a bitBlt to the PACKED16). Note that WinG always creates
	...
	if (dc->mi.bitsPerPixel == 15) {
		...
		info.maskRed = 0x0000F800;
		info.maskGreen = 0x000007E0;
		info.maskBlue = 0x0000001F;
		}
...
void PACKED16_initInternal(MGLDC *dc)
/****************************************************************************
*
* Function:		PACKED16_initInternal
* Parameters:	dc	- Device context.
...
****************************************************************************/
{
	...
	dc->r.setColor 				= PACKED16_setColor;
	dc->r.setBackColor 			= PACKED16_setBackColor;
	...
	_MGL_setLineVecs(dc,		  PACKED16_line);
	dc->r.solid.scanLine 		= PACKED16_solidScanLine;
	dc->r.ropSolid.scanLine 	= PACKED16_ropSolidScanLine;
	_MGL_setFillRectVecs(dc,	  __EMU__fillRect);
	dc->r.solid.fillRect		= PACKED16_solidFillRect;
	dc->r.putMonoImage			= PACKED16_putMonoImage;
	dc->r.putMouseImage			= __EMU__putMouseImage;
	dc->r.getImage				= PACKED16_getImage;
	dc->r.putImage				= PACKED16_putImage;
	dc->r.divotSize				= PACKED16_divotSize;
	dc->r.getDivot 				= PACKED16_getDivot;
	dc->r.putDivot 				= PACKED16_putDivot;
	dc->r.stretchScanLine2x		= PACKED16_stretchScanLine2x;
	dc->r.stretchScanLine		= PACKED16_stretchScanLine;
	dc->r.stretchBlt1x2			= PACKED16_stretchBlt1x2;
	dc->r.stretchBlt2x2			= PACKED16_stretchBlt2x2;
	dc->r.stretchBlt			= PACKED16_stretchBlt;

#ifndef	MGL_LITE
	dc->r.stippleLine			= PACKED16_stippleLine;
	_MGL_setTrapVecs(dc,		  __EMU__trap);
	dc->r.solid.trap			= PACKED16_solidTrap;
	dc->r.putSrcTransparent		= PACKED16_putSrcTransparent;
	dc->r.putDstTransparent		= PACKED16_putDstTransparent;
	...
	dc->r.patt.scanLine			= PACKED16_pattScanLine;
	dc->r.colorPatt.scanLine 	= PACKED16_colorPattScanLine; 	
	dc->r.fatSolid.scanLine 	= PACKED16_solidScanLine;
	dc->r.fatRopSolid.scanLine 	= PACKED16_ropSolidScanLine;  	
	dc->r.fatPatt.scanLine 		= PACKED16_pattScanLine;	  	
	dc->r.fatColorPatt.scanLine = PACKED16_colorPattScanLine; 	
	_MGL_setDrawScanListVecs(dc,  __EMU__drawScanList);
	dc->r.solid.drawScanList 	= PACKED16_solidDrawScanList;
...
//		 C using the new floating point formulas for speed.
	_MGL_setEllipseVecs(dc,	  	  __EMU__ellipse);
//	_MGL_setEllipseVecs(dc,	  	  PACKED16_ellipse);
#endif
	...
	dc->r.translateImage		= PACKED16_translateImage;
	dc->r.bitBlt				= __EMU__bitBlt;
	dc->r.srcTransBlt			= PACKED16_srcTransBlt;
	dc->r.dstTransBlt			= PACKED16_dstTransBlt;
	dc->r.scanRightForColor		= PACKED16_scanRightForColor;
	dc->r.scanLeftForColor		= PACKED16_scanLeftForColor;
	dc->r.scanRightWhileColor	= PACKED16_scanRightWhileColor;
	dc->r.scanLeftWhileColor	= PACKED16_scanLeftWhileColor;

#ifdef	MGL_FIX3D
	/* 3D rasterisation */
	dc->r.cLine 				= PACKED16_cLine;
	dc->r.tri 					= __EMU__tri;
	...
	dc->r.cTrap					= PACKED16_cTrap;

	/* 3D 16 bit Zbuffering */
	dc->r.zBegin				= __EMU__zBegin;
	dc->r.z16.zClear			= __EMU__zClear16;
	dc->r.z16.zLine				= PACKED16_zLine16;
	dc->r.z16.czLine			= PACKED16_czLine16;
	...
	dc->r.z16.zTrap				= PACKED16_zTrap16;
	dc->r.z16.czTrap			= PACKED16_czTrap16;

	/* 3D 32 bit Zbuffering */
	dc->r.z32.zClear			= __EMU__zClear32;
	dc->r.z32.zLine				= PACKED16_zLine32;
	dc->r.z32.czLine			= PACKED16_czLine32;
	...
	dc->r.z32.zTrap				= PACKED16_zTrap32;
	dc->r.z32.czTrap			= PACKED16_czTrap32;

	/* Special versions for 15/16 bit color */
	if (dc->mi.bitsPerPixel == 15) {
		dc->r.rgbLine 			= PACKED15_rgbLine;
		dc->r.rgbTrap			= PACKED15_rgbTrap;
		dc->r.z16.rgbzLine		= PACKED15_rgbzLine16;
		dc->r.z32.rgbzLine		= PACKED15_rgbzLine32;
		dc->r.z16.rgbzTrap		= PACKED15_rgbzTrap16;
		dc->r.z32.rgbzTrap		= PACKED15_rgbzTrap32;
		}
	else {
		dc->r.rgbLine 			= PACKED16_rgbLine;
		dc->r.rgbTrap			= PACKED16_rgbTrap;
		dc->r.z16.rgbzLine		= PACKED16_rgbzLine16;
		dc->r.z32.rgbzLine		= PACKED16_rgbzLine32;
		dc->r.z16.rgbzTrap		= PACKED16_rgbzTrap16;
		dc->r.z32.rgbzTrap		= PACKED16_rgbzTrap32;
		}
#endif	/* MGL_FIX3D */
#endif	/* !MGL_LITE */
}

ibool _ASMAPI PACKED16_initDriver(MGLDC *dc,int driverId,int modeId,ulong hwnd,
	int virtualX,int virtualY,int numBuffers,ibool stereo,int refreshRate)
/****************************************************************************
*
* Function:		PACKED16_initDriver
* Parameters:	dc	- Device context.
* Returns:		True if the device was correctly initialised.
*
...
****************************************************************************/
{
	...
	if (g_state == NULL) {
		if ((g_state = (void*)MGL_calloc(1,sizeof(PACKED16_state))) == NULL) {
			_MGL_result = grNoMem;
			return false;
			}
		}
	...
	dc->mi.maxColor = (dc->mi.bitsPerPixel == 15) ? 0x7FFF : 0xFFFF;
	dc->mi.maxPage = 0;
...
void _ASMAPI PACKED16_setColor(color_t color)
{
	DC.a.color = color;
	DC.intColor = color | (color << 16);
}

void _ASMAPI PACKED16_setBackColor(color_t color)
{
	DC.a.backColor = color;
	DC.intBackColor = color | (color << 16);
}
...
color_t _ASMAPI PACKED16_getPixel(int x,int y)
/****************************************************************************
*
* Function:		PACKED16_getPixel
* Parameters:	x,y	- Coordinate to read pixel from
* Returns:		Color of the pixel at the specified coordinate
*
****************************************************************************/
{
	return *(ushort*)PACKED16_pixelAddr(x,y);
}

void _ASMAPI PACKED16_putPixel(int x,int y)
/****************************************************************************
*
* Function:		PACKED16_putPixel
* Parameters:	x,y	- Coordinate to read pixel from
*
****************************************************************************/
{
	...
	s = PACKED16_pixelAddr(x,y);
	if (DC.a.writeMode == MGL_REPLACE_MODE)
		*s = (ushort)DC.intColor;
...
void _ASMAPI PACKED16_getScanLine(MGLDC *dc,int y,int x1,int x2,void *buffer,
	int bufx,int op)
/****************************************************************************
*
* Function:		PACKED16_getScanLine
* Parameters:	dc		- Device context
...
****************************************************************************/
{
	...
	s = PACKED16_pixelAddr2(dc,x1,y);
	if (op == MGL_REPLACE_MODE)
		memcpy(p,s,numPixels<<1);
...
void _ASMAPI PACKED16_putScanLine(MGLDC *dc,int y,int x1,int x2,void *buffer,
	int bufx,int op)
/****************************************************************************
*
* Function:		PACKED16_putScanLine
* Parameters:	dc		- Device context
*				y		- Scanline to get data from
*				x1		- Starting coordinate in scanline
*				y2		- Ending coordinate in scanline
...
void _ASMAPI PACKED16_line(fix32_t x1,fix32_t y1,fix32_t x2,
	fix32_t y2)
/****************************************************************************
*
* Function:		PACKED16_line
...
void _ASMAPI PACKED16_solidScanLine(int y,int x1,int x2)
/****************************************************************************
*
* Function:		PACKED16_solidScanLine
...
****************************************************************************/
{
	...
	if (x2 < x1)
		SWAPT(x1,x2,i);
	d = PACKED16_pixelAddr(x1,y);
	if ((numPixels = (x2-x1)) >= 8) {
		/* Force DWORD alignment */
		if (((uint)d & 1) != 0) {
			*d++ = (ushort)DC.intColor;
			numPixels--;
			}
		count = numPixels >> 1;
		numPixels &= 1;
...
void _ASMAPI PACKED16_ropSolidScanLine(int y,int x1,int x2)
/****************************************************************************
*
* Function:		PACKED16_ropSolidScanLine
* Parameters:	y		- Scanline to draw line at
*				x1,x2	- Extent of the scanline to draw
*
* Description:	Draws a scanline by plotting each pixel on the scanline
...
void _ASMAPI PACKED16_pattScanLine(int y,int x1,int x2)
/****************************************************************************
*
* Function:		PACKED16_pattScanLine
* Parameters:	y		- Scanline to draw line at
*				x1,x2	- Extent of the scanline to draw
*
* Description:	Draws a scanline by plotting each pixel on the scanline
...
void _ASMAPI PACKED16_solidFillRect(int x1,int y1,int x2,int y2)
/****************************************************************************
*
* Function:		PACKED16_solidFillRect
...
****************************************************************************/
{
	int		y,dInc = DC.mi.bytesPerLine - (x2-x1)*2;
	ushort	*d;

	d = PACKED16_pixelAddr(x1,y1);
	for (y = y1; y < y2; y++) {
		int count,numPixels;
		numPixels = (x2-x1);
		if (numPixels >= 8) {
...
void _ASMAPI PACKED16_putMonoImage(MGLDC *dc,int x,int y,int byteWidth,
	int height,uchar *image)
/****************************************************************************
*
* Function:		PACKED16_putMonoImage
* Parameters:	dc		- Device context
...
****************************************************************************/
{
	...
	ushort	*d,*dScanEnd,*dEnd,byte,color;

	/* Setup initial variables */
	clipRect = dc->intClipRect;
	right = x + (width = byteWidth << 3);
	bottom = y + height;
	color = (ushort)dc->intColor;
	d = PACKED16_pixelAddr2(dc,x,y);
	dEnd = d + ((height-1) * dc->mi.bytesPerLine/2) + width;
	dInc = dc->mi.bytesPerLine - width*2;
	op = dc->a.writeMode;
...
void _ASMAPI PACKED16_putImage(MGLDC *dc,int left,int top,int right,int bottom,
	int dstLeft,int dstTop,int op,void *surface,int bytesPerLine, MGLDC *src)
/****************************************************************************
*
* Function:		PACKED16_putImage
...
****************************************************************************/
{
	...
	s = PACKED16_pixelAddr3(src,left,top,surface,bytesPerLine);
	sInc = bytesPerLine;
	d = PACKED16_pixelAddr2(dc,dstLeft,dstTop);
...
void _ASMAPI PACKED16_getDivot(MGLDC *dc,int left,int top,int right,int bottom,
	void *divot)
/****************************************************************************
*
* Function:		PACKED16_getDivot
* Parameters:	dc		- Device context
...
****************************************************************************/
{
	...
	s = PACKED16_pixelAddr2(dc,left,top);
...
void _ASMAPI PACKED16_putDivot(MGLDC *dc,void *divot)
/****************************************************************************
*
* Function:		PACKED16_putDivot
* Parameters:	dc		- Device context
...
****************************************************************************/
{
	...
	d = PACKED16_pixelAddr2(dc,hdr.left,hdr.top);
	dInc = dc->mi.bytesPerLine;
...
void _ASMAPI PACKED16_cLine(fix32_t x1,fix32_t y1,fix32_t c1,fix32_t x2,
	fix32_t y2,fix32_t c2)
/****************************************************************************
*
* Function:		PACKED16_cLine
...
void _ASMAPI PACKED16_zLine16(fix32_t x1,fix32_t y1,zfix32_t z1,fix32_t x2,
	fix32_t y2,zfix32_t z2)
/****************************************************************************
*
* Function:		PACKED16_zLine16
...
void _ASMAPI PACKED16_zLine32(fix32_t x1,fix32_t y1,zfix32_t z1,fix32_t x2,
	fix32_t y2,zfix32_t z2)
/****************************************************************************
*
* Function:		PACKED16_zLine32
* Parameters:	x1,y1,z1	- First endpoint for line
*				x2,y2,z2	- Second endpoint for line
*
* Description:  Scan convert a 3D line segment using the MidPoint Digital
...
void _ASMAPI PACKED16_czLine16(fix32_t x1,fix32_t y1,zfix32_t z1,fix32_t c1,
	fix32_t x2,fix32_t y2,zfix32_t z2,fix32_t c2)
/****************************************************************************
*
* Function:		PACKED16_czLine16
...
void _ASMAPI PACKED16_czLine32(fix32_t x1,fix32_t y1,zfix32_t z1,fix32_t c1,
	fix32_t x2,fix32_t y2,zfix32_t z2,fix32_t c2)
/****************************************************************************
*
* Function:		PACKED16_czLine32
* Parameters:	x1,y1,z1,c1	- First endpoint for line
*				x2,y2,z2,c2	- Second endpoint for line
*
* Description:  Renders a shaded zbuffered line (non clipped).
...
void _ASMAPI PACKED16_solidDrawScanList(int y,int length,short *scans)
/****************************************************************************
*
* Function:		PACKED16_solidDrawScanList
...
void _ASMAPI PACKED16_translateImage(MGLDC *dc,int left,int top,int right,
...
/****************************************************************************
*
* Function:		PACKED16_translateImage
* Parameters:	dc				- Device context to Blt to
* 				left			- Left coordinate of are to Blt from
*				top				- Top coordinate of image to Blt from
*				right			- Right coordinate of image to Blt from
...
****************************************************************************/
{
	...
	dInc = dc->mi.bytesPerLine - width*2;
	if (dc->mi.bitsPerPixel == 15) {
		/* Pack 24 bit RGB to 5/5/5 format */
		while (height--) {
			int count = width;
			while (count--) {
				*d++ = MGL_pack24to15(s);
		...
		/* Pack 24 bit RGB to 5/6/5 format */
		while (height--) {
			int count = width;
			while (count--) {
				*d++ = MGL_pack24to16(s);
...
void _ASMAPI PACKED16_srcTransBlt(MGLDC *dst,MGLDC *src,int left,int top,
	int right,int bottom,int dstLeft,int dstTop,int op,color_t transparent)
/****************************************************************************
*
* Function:		PACKED16_srcTransBlt
...
****************************************************************************/
{
	...
	d = PACKED16_pixelAddr2(dst,dstLeft,dstTop);
	s = PACKED16_pixelAddr2(src,left,top);
	height = bottom - top;
	width = right - left;
	sInc = src->mi.bytesPerLine - width*2;
	dInc = dst->mi.bytesPerLine - width*2;
	while (height--) {
		int count = width;
		while (count--) {
			if (*s != (ushort)transparent)
				*d = *s;
			d++;
			s++;
			}
...
void _ASMAPI PACKED16_dstTransBlt(MGLDC *dst,MGLDC *src,int left,int top,
	int right,int bottom,int dstLeft,int dstTop,int op,color_t transparent)
/****************************************************************************
*
* Function:		PACKED16_dstTransBlt
* Parameters:	dst			- Destination Device context
*				src			- Source device context
* 				left		- Left coordinate of are to Blt from
*				top			- Top coordinate of image to Blt from
...
int _ASMAPI PACKED16_scanRightForColor(int x,int y,color_t color)
/****************************************************************************
*
* Function:		PACKED16_scanRightForColor
* Parameters:	dc		- Device context
...
int _ASMAPI PACKED16_scanLeftForColor(int x,int y,color_t color)
/****************************************************************************
*
* Function:		PACKED16_scanLeftForColor
* Parameters:	dc		- Device context
*				x,y		- Coordinate to begin scanning at
*				color	- Color to scan for
...
int _ASMAPI PACKED16_scanRightWhileColor(int x,int y,color_t color)
/****************************************************************************
*
* Function:		PACKED16_scanRightWhileColor
* Parameters:	dc		- Device context
*				x,y		- Coordinate to begin scanning at
...
****************************************************************************/
{
	...
	p = PACKED16_pixelAddr(x,y);
	while (x <= DC.mi.xRes && (*p == (ushort)color)) {
		x++;
		p++;
		}
	return x;
...
int _ASMAPI PACKED16_scanLeftWhileColor(int x,int y,color_t color)
/****************************************************************************
*
* Function:		PACKED16_scanLeftWhileColor
* Parameters:	dc		- Device context
*				x,y		- Coordinate to begin scanning at
*				color	- Color to continue scanning on
* Returns:      Coordinate of pixel, or -1 if not found
...
void _ASMAPI PACKED16_solidTrap(void)
/****************************************************************************
*
* Function:		PACKED16_solidTrap
*
...
****************************************************************************/
{
	...
	ushort		*fbufstart,*fbuf;

	fbufstart = PACKED16_pixelAddr(0,DC.tr.y);
	while (DC.tr.count--) {
...
void _ASMAPI PACKED16_cTrap(void)
/****************************************************************************
*
* Function:		PACKED16_cTrap
*
...
void _ASMAPI PACKED16_zTrap16(void)
/****************************************************************************
*
* Function:		PACKED16_zTrap16
*
...
void _ASMAPI PACKED16_zTrap32(void)
/****************************************************************************
*
* Function:		PACKED16_zTrap32
*
* Description:	Renders a flat shaded zbuffered trapezoid. The parameters
*				for the trapezoid are found in the DC.tr structure.
*
...
void _ASMAPI PACKED16_czTrap16(void)
/****************************************************************************
*
* Function:		PACKED16_czTrap16
*
...
void _ASMAPI PACKED16_czTrap32(void)
/****************************************************************************
*
* Function:		PACKED16_czTrap32
*
* Description:	Renders a smooth shaded color index zbuffered trapezoid. The
*				parameters for the trapezoid are found in the DC.tr
*				structure.
...
void _ASMAPI PACKED16_stretchBlt2x2_24(MGLDC *dst,MGLDC *src,int left,int top,
	int right,int bottom,int dstLeft,int dstTop)
/****************************************************************************
*
* Function:		PACKED16_stretchBlt2x2_24
* Parameters:	dst		- Destination Device context
*				src		- Source device context
* 				left	- Left coordinate of are to Blt from
*				top		- Top coordinate of image to Blt from
...
****************************************************************************/
{
	...
	count = bottom - top;
	dBegin = PACKED16_pixelAddr2(dst,dstLeft,dstTop);
	sBegin = PACKED24_pixelAddr2(src,left,top);
	while (count--) {
		uchar *s = sBegin;
		ushort *d = dBegin;
		int count = xcount;
		if (dst->mi.bitsPerPixel == 15) {
			while (count--) {
				d[0] = d[1] = MGL_pack24to15(s);
				d += 2;
				s += 3;
				}
			}
...
void _ASMAPI PACKED16_stretchBlt2x2(MGLDC *dst,MGLDC *src,int left,int top,
	int right,int bottom,int dstLeft,int dstTop,palette_t *pal,ibool idPal)
/****************************************************************************
*
* Function:		PACKED16_stretchBlt2x2
* Parameters:	dst		- Destination Device context
*				src		- Source device context
*				top		- Top coordinate of image to Blt from
*				right	- Right coordinate of image to Blt from
...
****************************************************************************/
{
	...
	if (src->mi.bitsPerPixel != dst->mi.bitsPerPixel) {
		if (src->mi.bitsPerPixel == 24 && src->pf.redPos != 0)
			PACKED16_stretchBlt2x2_24(dst,src,left,top,right,bottom,dstLeft,
				dstTop);
		else
...
long _ASMAPI PACKED16_divotSize(MGLDC *dc,int left,int top,int right,
	int bottom)
/****************************************************************************
*
* Function:     PACKED16_divotSize
...
void _ASMAPI PACKED16_stretchScanLine2x(MGLDC *dc,int y,int x1,int count,
	void *buffer)
/****************************************************************************
*
* Function:		PACKED16_stretchScanline2x
* Parameters:	dc			- Device context
*				y			- Scanline to get data from
*				x1			- Starting coordinate in scanline
...
void _ASMAPI PACKED16_stretchScanLine(MGLDC *dc,int y,int x1,int count,
	void *buffer,int *repCounts)
/****************************************************************************
*
* Function:		PACKED16_stretchScanline
* Parameters:	dc			- Device context
*				y			- Scanline to get data from
*				x1			- Starting coordinate in scanline
*				count		- Number of source pixels
...
void _ASMAPI PACKED16_putSrcTransparent(MGLDC *dc,int y,int x1,int x2,
	void *buffer,color_t transparent)
/****************************************************************************
*
* Function:		PACKED16_putSrcTransparent
* Parameters:	dc			- Device context
*				y			- Scanline to get data from
*				x1			- Starting coordinate in scanline
*				y2			- Ending coordinate in scanline
...
{
	int		count = (x2 - x1);
	ushort	*s = buffer,*p,t = transparent;
...
void _ASMAPI PACKED16_putDstTransparent(MGLDC *dc,int y,int x1,int x2,
	void *buffer,color_t transparent)
/****************************************************************************
*
* Function:		PACKED16_putDstTransparent
* Parameters:	dc			- Device context
*				y			- Scanline to get data from
*				x1			- Starting coordinate in scanline
*				y2			- Ending coordinate in scanline
...
void _ASMAPI PACKED16_getImage(MGLDC *dc,int left,int top,int right,int bottom,
	int dstLeft,int dstTop,int op,void *surface,int bytesPerLine)
/****************************************************************************
*
* Function:		PACKED16_getImage
...
****************************************************************************/
{
	...
	s = PACKED16_pixelAddr2(dc,left,top);
	sInc = dc->mi.bytesPerLine;
	d = PACKED16_pixelAddr3(src,dstLeft,dstTop,surface,bytesPerLine);
	dInc = bytesPerLine;
...
void _ASMAPI PACKED16_colorPattScanLine(int y,int x1,int x2)
/****************************************************************************
*
* Function:		PACKED16_colorPattScanLine
* Parameters:	y		- Scanline to draw line at
*				x1,x2	- Extent of the scanline to draw
*
* Description:	Draws a scanline by plotting each pixel on the scanline
...
void _ASMAPI PACKED16_stretchBlt1x2_24(MGLDC *dst,MGLDC *src,int left,int top,
	int right,int bottom,int dstLeft,int dstTop)
/****************************************************************************
*
* Function:		PACKED16_stretchBlt1x2_24
* Parameters:	dst		- Destination Device context
*				src		- Source device context
* 				left	- Left coordinate of are to Blt from
*				top		- Top coordinate of image to Blt from
...
void _ASMAPI PACKED16_stippleLine(fix32_t x1,fix32_t y1,fix32_t x2,fix32_t y2)
/****************************************************************************
*
* Function:		PACKED16_stippleLine
* Parameters:	x1,y1	- First endpoint for line
*				x2,y2	- Second endpoint for line
*
...
void _ASMAPI PACKED16_stretchBlt1x2(MGLDC *dst,MGLDC *src,int left,int top,
	int right,int bottom,int dstLeft,int dstTop,palette_t *pal,ibool idPal)
/****************************************************************************
*
* Function:		PACKED16_stretchBlt1x2
* Parameters:	dst		- Destination Device context
*				src		- Source device context
* 				left	- Left coordinate of are to Blt from
*				top		- Top coordinate of image to Blt from
...
****************************************************************************/
{
	...
	if (src->mi.bitsPerPixel != dst->mi.bitsPerPixel) {
		if (src->mi.bitsPerPixel == 24 && src->pf.redPos != 0)
			PACKED16_stretchBlt1x2_24(dst,src,left,top,right,bottom,dstLeft,dstTop);
		else
	...
	count = bottom - top;
	dBegin = PACKED16_pixelAddr2(dst,dstLeft,dstTop);
	sBegin = PACKED16_pixelAddr2(src,left,top);
...
void _ASMAPI PACKED16_stretchBlt(MGLDC *dst,MGLDC *src,int left,int top,
	int right,int bottom,int dstLeft,int dstTop,fix32_t zoomx,fix32_t zoomy,
	palette_t *pal,ibool idPal)
/****************************************************************************
*
* Function:		PACKED16_stretchBlt
* Parameters:	dst		- Destination Device context
*				src		- Source device context
* 				left	- Left coordinate of are to Blt from
*				top		- Top coordinate of image to Blt from
...
****************************************************************************/
{
	...
	dBegin = PACKED16_pixelAddr2(dst,dstLeft,dstTop);
	if (src->mi.bitsPerPixel == dst->mi.bitsPerPixel) {
		ushort *sBegin = PACKED16_pixelAddr2(src,left,top);
		while (count--) {
...
#else
				PACKED16_stretchScan(dBegin,sBegin,xcount,repCounts);
#endif
				BINC(dBegin,dstBytes);
				}
			BINC(sBegin,srcBytes);
...
#else
					PACKED15_stretchScan24(dBegin,sBegin,xcount,repCounts);
#endif
					BINC(dBegin,dstBytes);
					}
				BINC(sBegin,srcBytes);
...
#else
					PACKED16_stretchScan24(dBegin,sBegin,xcount,repCounts);
#endif
					BINC(dBegin,dstBytes);
					}
				BINC(sBegin,srcBytes);
...
void _ASMAPI PACKED15_rgbLine(fix32_t x1,fix32_t y1,fix32_t r1,fix32_t g1,
	fix32_t b1,fix32_t x2,fix32_t y2,fix32_t r2,fix32_t g2,fix32_t b2)
/****************************************************************************
*
* Function:		PACKED15_rgbLine
...
****************************************************************************/
{
	...
	if (lowslope) {
		...
		while (count--) {
			*fbuf++ = MGL_packColor15FX(r1,g1,b1);
			r1 += rSlope;
			g1 += gSlope;
			b1 += bSlope;
...
void _ASMAPI PACKED15_rgbzLine16(fix32_t x1,fix32_t y1,zfix32_t z1,
	fix32_t r1,fix32_t b1,fix32_t g1,fix32_t x2,fix32_t y2,zfix32_t z2,
	fix32_t r2,fix32_t b2,fix32_t g2)
/****************************************************************************
*
* Function:		PACKED15_rgbzLine16
* Parameters:	x1,y1,r1,g1,b1	- First endpoint for line
*				x2,y2,r2,g2,b2	- Second endpoint for line
*
* Description:  Renders an RGB shaded line (non clipped).
...
void _ASMAPI PACKED15_rgbzLine32(fix32_t x1,fix32_t y1,zfix32_t z1,
	fix32_t r1,fix32_t b1,fix32_t g1,fix32_t x2,fix32_t y2,zfix32_t z2,
	fix32_t r2,fix32_t b2,fix32_t g2)
/****************************************************************************
*
* Function:		PACKED15_rgbzLine32
* Parameters:	x1,y1,r1,g1,b1	- First endpoint for line
*				x2,y2,r2,g2,b2	- Second endpoint for line
*
* Description:  Renders an RGB shaded line (non clipped).
...
void _ASMAPI PACKED16_rgbLine(fix32_t x1,fix32_t y1,fix32_t r1,fix32_t g1,
	fix32_t b1,fix32_t x2,fix32_t y2,fix32_t r2,fix32_t g2,fix32_t b2)
/****************************************************************************
*
* Function:		PACKED16_rgbLine
* Parameters:	x1,y1,r1,g1,b1	- First endpoint for line
*				x2,y2,r2,g2,b2	- Second endpoint for line
*
* Description:  Renders an RGB shaded line (non clipped).
...
****************************************************************************/
{
	...
	if (lowslope) {
		...
		while (count--) {
			*fbuf++ = MGL_packColor16FX(r1,g1,b1);
			r1 += rSlope;
			g1 += gSlope;
			b1 += bSlope;
...
void _ASMAPI PACKED16_rgbzLine16(fix32_t x1,fix32_t y1,zfix32_t z1,fix32_t r1,
	fix32_t b1,fix32_t g1,fix32_t x2,fix32_t y2,zfix32_t z2,fix32_t r2,
	fix32_t b2,fix32_t g2)
/****************************************************************************
*
* Function:		PACKED16_rgbzLine16
* Parameters:	x1,y1,r1,g1,b1	- First endpoint for line
*				x2,y2,r2,g2,b2	- Second endpoint for line
*
* Description:  Renders an RGB shaded line (non clipped).
...
void _ASMAPI PACKED16_rgbzLine32(fix32_t x1,fix32_t y1,zfix32_t z1,fix32_t r1,
	fix32_t b1,fix32_t g1,fix32_t x2,fix32_t y2,zfix32_t z2,fix32_t r2,
	fix32_t b2,fix32_t g2)
/****************************************************************************
*
* Function:		PACKED16_rgbzLine32
* Parameters:	x1,y1,r1,g1,b1	- First endpoint for line
*				x2,y2,r2,g2,b2	- Second endpoint for line
*
* Description:  Renders an RGB shaded line (non clipped).
...
void _ASMAPI PACKED15_rgbTrap(void)
/****************************************************************************
*
* Function:		PACKED15_rgbTrap
*
...
void _ASMAPI PACKED15_rgbzTrap16(void)
/****************************************************************************
*
* Function:		PACKED15_rgbzTrap16
*
* Description:	Renders a trapezoid which is a flat topped/bottom polygon
*				with two edges.
*
...
void _ASMAPI PACKED15_rgbzTrap32(void)
/****************************************************************************
*
* Function:		PACKED15_rgbzTrap32
*
* Description:	Renders a trapezoid which is a flat topped/bottom polygon
*				with two edges.
*
...
void _ASMAPI PACKED16_rgbTrap(void)
/****************************************************************************
*
* Function:		PACKED16_rgbTrap
*
* Description:	Renders a trapezoid which is a flat topped/bottom polygon
*				with two edges.
*
****************************************************************************/
{
	...
	while (DC.tr.count--) {
		...
		if (dX != 0) {
			...
			bSlope = MGL_FixDiv(ib2 - ib1,dx);
			while (dX--) {
				*fbuf = MGL_packColor16FX(ir1,ig1,ib1);
				ir1 += rSlope;	ig1 += gSlope;	ib1 += bSlope;
				fbuf++;
				}
			}
...
void _ASMAPI PACKED16_rgbzTrap16(void)
/****************************************************************************
*
* Function:		PACKED16_rgbzTrap16
*
* Description:	Renders a trapezoid which is a flat topped/bottom polygon
*				with two edges.
*
...
void _ASMAPI PACKED16_rgbzTrap32(void)
/****************************************************************************
*
* Function:		PACKED16_rgbzTrap32
*
* Description:	Renders a trapezoid which is a flat topped/bottom polygon
*				with two edges.
*
