...
*
* Description:	Main SuperVGA 32 bit 16m color device driver file. This file
...
#include "mgl.h"
#pragma hdrstop
#include "drivers\svga\svga32.h"
#include "drivers\packed\packed32.h"

/*---------------------------- Driver Tables -----------------------------*/

drivertype	_VARAPI SVGA32_driver = {
	IDENT_FLAG,
	MGL_SVGA32NAME,
	MGL_SVGA32NAME,
...
PRIVATE gmode_t SVGA32_modes[] = {
	...
	{1151,864,32,1,0xFFFFFFFFL,0,0,1000,0,0x4F02},
...
ibool _ASMAPI SVGA32_detect(int id,int force,int *driver,int *mode,modetab modes)
/****************************************************************************
*
* Function:		SVGA32_detect
...
****************************************************************************/
{
	if (force >= grVESA && SVGA_detect(driver)) {
		 ...
		 */
		SVGA_useMode(modes,grSVGA_320x200x4G,id,&SVGA32_modes[0]);
		SVGA_useMode(modes,grSVGA_320x240x4G,id,&SVGA32_modes[1]);
		SVGA_useMode(modes,grSVGA_320x400x4G,id,&SVGA32_modes[2]);
		SVGA_useMode(modes,grSVGA_320x480x4G,id,&SVGA32_modes[3]);
		SVGA_useMode(modes,grSVGA_400x300x4G,id,&SVGA32_modes[4]);
		SVGA_useMode(modes,grSVGA_512x384x4G,id,&SVGA32_modes[5]);
		SVGA_useMode(modes,grSVGA_640x350x4G,id,&SVGA32_modes[6]);
		SVGA_useMode(modes,grSVGA_640x400x4G,id,&SVGA32_modes[7]);
		SVGA_useMode(modes,grSVGA_640x480x4G,id,&SVGA32_modes[8]);
		SVGA_useMode(modes,grSVGA_800x600x4G,id,&SVGA32_modes[9]);
		SVGA_useMode(modes,grSVGA_1024x768x4G,id,&SVGA32_modes[10]);
		SVGA_useMode(modes,grSVGA_1152x864x4G,id,&SVGA32_modes[11]);
		SVGA_useMode(modes,grSVGA_1280x960x4G,id,&SVGA32_modes[12]);
		SVGA_useMode(modes,grSVGA_1280x1024x4G,id,&SVGA32_modes[13]);
		SVGA_useMode(modes,grSVGA_1600x1200x4G,id,&SVGA32_modes[14]);
		return true;
		}
	mode = mode;
	return false;
...
****************************************************************************/
{
	...
	dc->r.getPixel 				= SVGA32_getPixel;
	dc->r.putPixel 				= SVGA32_putPixel;
	dc->r.getScanLine			= SVGA32_getScanLine;
	dc->r.putScanLine			= SVGA32_putScanLine;
	dc->r.stretchScanLine		= SVGA32_stretchScanLine;
	dc->r.stretchScanLine2x		= SVGA32_stretchScanLine2x;
	...
	dc->r.putSrcTransparent		= SVGA32_putSrcTransparent;
	dc->r.putDstTransparent		= SVGA32_putDstTransparent;
#endif

	/* Fill in remaining device vectors */
	_MGL_setLineVecs(dc,	  SVGA32_line);
	_MGL_setScanLineVecs(dc,  SVGA32_scanLine);
	_MGL_setFillRectVecs(dc,  SVGA32_fillRect);
	dc->r.putMonoImage			= SVGA32_putMonoImage;
	dc->r.putMouseImage			= __EMU__putMouseImage;
	dc->r.getImage				= __EMU__getImage;
#ifdef  __16BIT__
	dc->r.putImage				= __EMU__putImage;
#else
	dc->r.putImage				= SVGA32_putImage;
#endif
	dc->r.divotSize				= SVGA32_divotSize;
	dc->r.getDivot 				= SVGA32_getDivot;
	dc->r.putDivot 				= SVGA32_putDivot;
	...
	_MGL_setDrawScanListVecs(dc,SVGA32_drawScanList);
	...
	dc->r.scanRightForColor		= SVGA32_scanRightForColor;
	dc->r.scanLeftForColor		= SVGA32_scanLeftForColor;
	dc->r.scanRightWhileColor	= SVGA32_scanRightWhileColor;
	dc->r.scanLeftWhileColor	= SVGA32_scanLeftWhileColor;
...
ibool _ASMAPI SVGA32_initDriver(MGLDC *dc,int driverId,int modeId,ulong hwnd,
	int virtualX,int virtualY,int numBuffers,ibool stereo,int refreshRate)
/****************************************************************************
*
* Function:		SVGA32_initDriver
* Parameters:	dc	- Device context.
* Returns:		True if the device was correctly initialised.
*
...
****************************************************************************/
{
	...
	dc->mi = SVGA32_modes[modeId - grSVGA_320x200x4G];
	if (g_state == NULL) {
...
long _ASMAPI SVGA32_divotSize(MGLDC *dc,int left,int top,int right,
	int bottom)
/****************************************************************************
*
* Function:		SVGA32_divotSize
