...
*
* Description:	Main Linear Framebuffer SuperVGA 24 bit device driver.
*
*
****************************************************************************/
...
#include "mgl.h"
#pragma hdrstop
#include "drivers\svga\linear24.h"
...
drivertype	_VARAPI LINEAR24_driver = {
	IDENT_FLAG,
	MGL_LINEAR24NAME,
	MGL_LINEAR24NAME,
...
PRIVATE gmode_t LINEAR24_modes[] = {
...
ibool _ASMAPI LINEAR24_detect(int id,int force,int *driver,int *mode,modetab modes)
/****************************************************************************
*
* Function:		LINEAR24_detect
...
****************************************************************************/
{
	if (force >= grVESA && SVGA_detect(driver)) {
		 ...
		 */
		SVGALIN_useMode(modes,grSVGA_320x200x16m,id,&LINEAR24_modes[0],false);
		SVGALIN_useMode(modes,grSVGA_320x240x16m,id,&LINEAR24_modes[1],false);
		SVGALIN_useMode(modes,grSVGA_320x400x16m,id,&LINEAR24_modes[2],false);
		SVGALIN_useMode(modes,grSVGA_320x480x16m,id,&LINEAR24_modes[3],false);
		SVGALIN_useMode(modes,grSVGA_400x300x16m,id,&LINEAR24_modes[4],false);
		SVGALIN_useMode(modes,grSVGA_512x384x16m,id,&LINEAR24_modes[5],false);
		SVGALIN_useMode(modes,grSVGA_640x350x16m,id,&LINEAR24_modes[6],false);
		SVGALIN_useMode(modes,grSVGA_640x400x16m,id,&LINEAR24_modes[7],false);
		SVGALIN_useMode(modes,grSVGA_640x480x16m,id,&LINEAR24_modes[8],false);
		SVGALIN_useMode(modes,grSVGA_800x600x16m,id,&LINEAR24_modes[9],false);
		SVGALIN_useMode(modes,grSVGA_1024x768x16m,id,&LINEAR24_modes[10],false);
		SVGALIN_useMode(modes,grSVGA_1152x864x16m,id,&LINEAR24_modes[11],false);
		SVGALIN_useMode(modes,grSVGA_1280x960x16m,id,&LINEAR24_modes[12],false);
		SVGALIN_useMode(modes,grSVGA_1280x1024x16m,id,&LINEAR24_modes[13],false);
		SVGALIN_useMode(modes,grSVGA_1600x1200x16m,id,&LINEAR24_modes[14],false);
		return true;
		}
	mode = mode;
	return false;
...
void LINEAR24_initInternal(MGLDC *dc)
/****************************************************************************
*
* Function:		LINEAR24_initInternal
* Parameters:	dc	- Device context.
...
ibool _ASMAPI LINEAR24_initDriver(MGLDC *dc,int driverId,int modeId,ulong hwnd,
	int virtualX,int virtualY,int numBuffers,ibool stereo,int refreshRate)
/****************************************************************************
*
* Function:		LINEAR24_initDriver
* Parameters:	dc	- Device context.
* Returns:		True if the device was correctly initialised.
*
...
****************************************************************************/
{
	...
	dc->mi = LINEAR24_modes[modeId - grSVGA_320x200x16m];
	if (g_state == NULL) {
