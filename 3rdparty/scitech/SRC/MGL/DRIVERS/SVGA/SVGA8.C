...
*
* Description:	Main SuperVGA 256 color device driver file. This file
...
#include "mgl.h"
#pragma hdrstop
#include "drivers\svga\svga8.h"
#include "drivers\packed\packed8.h"

/*---------------------------- Driver Tables -----------------------------*/

drivertype	_VARAPI SVGA8_driver = {
	IDENT_FLAG,
	MGL_SVGA8NAME,
	MGL_SVGA8NAME,
...
ibool _ASMAPI SVGA8_detect(int id,int force,int *driver,int *mode,modetab modes)
/****************************************************************************
*
* Function:		SVGA8_detect
...
****************************************************************************/
{
	if (force >= grVESA && SVGA_detect(driver)) {
		 ...
		 */
		SVGA_useMode(modes,grSVGA_320x200x256,id,&SVGA8_modes[0]);
		SVGA_useMode(modes,grSVGA_320x240x256,id,&SVGA8_modes[1]);
		SVGA_useMode(modes,grSVGA_320x400x256,id,&SVGA8_modes[2]);
		SVGA_useMode(modes,grSVGA_320x480x256,id,&SVGA8_modes[3]);
		SVGA_useMode(modes,grSVGA_400x300x256,id,&SVGA8_modes[4]);
		SVGA_useMode(modes,grSVGA_512x384x256,id,&SVGA8_modes[5]);
		SVGA_useMode(modes,grSVGA_640x350x256,id,&SVGA8_modes[6]);
		SVGA_useMode(modes,grSVGA_640x400x256,id,&SVGA8_modes[7]);
		SVGA_useMode(modes,grSVGA_640x480x256,id,&SVGA8_modes[8]);
		SVGA_useMode(modes,grSVGA_800x600x256,id,&SVGA8_modes[9]);
		SVGA_useMode(modes,grSVGA_1024x768x256,id,&SVGA8_modes[10]);
		SVGA_useMode(modes,grSVGA_1152x864x256,id,&SVGA8_modes[11]);
		SVGA_useMode(modes,grSVGA_1280x960x256,id,&SVGA8_modes[12]);
		SVGA_useMode(modes,grSVGA_1280x1024x256,id,&SVGA8_modes[13]);
		SVGA_useMode(modes,grSVGA_1600x1200x256,id,&SVGA8_modes[14]);
		return true;
		}
	mode = mode;
	return false;
...
void SVGA8_initInternal(MGLDC *dc)
/****************************************************************************
*
* Function:		SVGA8_initInternal
* Parameters:	dc	- Device context.
...
****************************************************************************/
{
	...
	dc->r.getPixel 				= SVGA8_getPixel;
	dc->r.putPixel 				= SVGA8_putPixel;
	dc->r.getScanLine			= SVGA8_getScanLine;
	dc->r.putScanLine			= SVGA8_putScanLine;
	dc->r.stretchScanLine		= SVGA8_stretchScanLine;
	dc->r.stretchScanLine2x		= SVGA8_stretchScanLine2x;
	...
	dc->r.ditherPixel			= SVGA8_ditherPixel;
	dc->r.putSrcTransparent		= SVGA8_putSrcTransparent;
	dc->r.putDstTransparent		= SVGA8_putDstTransparent;
#endif

	/* Fill in remaining device vectors */
	_MGL_setLineVecs(dc,	  SVGA8_line);
	_MGL_setScanLineVecs(dc,  SVGA8_scanLine);
	_MGL_setFillRectVecs(dc,  SVGA8_fillRect);
	dc->r.putMonoImage			= SVGA8_putMonoImage;
	dc->r.putMouseImage			= __EMU__putMouseImage;
	dc->r.getImage				= __EMU__getImage;
#ifdef  __16BIT__
	dc->r.putImage				= __EMU__putImage;
#else
	dc->r.putImage				= SVGA8_putImage;
#endif
	dc->r.divotSize				= SVGA8_divotSize;
	dc->r.getDivot 				= SVGA8_getDivot;
	dc->r.putDivot 				= SVGA8_putDivot;
	...
	_MGL_setDrawScanListVecs(dc,SVGA8_drawScanList);
	...
	dc->r.scanRightForColor		= SVGA8_scanRightForColor;
	dc->r.scanLeftForColor		= SVGA8_scanLeftForColor;
	dc->r.scanRightWhileColor	= SVGA8_scanRightWhileColor;
	dc->r.scanLeftWhileColor	= SVGA8_scanLeftWhileColor;

#ifdef	MGL_FIX3D
	/* 3D rasterisation */
	dc->r.cLine 				= __EMU__cLine;
...
ibool _ASMAPI SVGA8_initDriver(MGLDC *dc,int driverId,int modeId,ulong hwnd,
	int virtualX,int virtualY,int numBuffers,ibool stereo,int refreshRate)
/****************************************************************************
*
* Function:		SVGA8_initDriver
* Parameters:	dc	- Device context.
* Returns:		True if the device was correctly initialised.
*
...
****************************************************************************/
{
	...
	dc->mi = SVGA8_modes[modeId - grSVGA_320x200x256];
	if (g_state == NULL) {
...
long _ASMAPI SVGA8_divotSize(MGLDC *dc,int left,int top,int right,
	int bottom)
/****************************************************************************
*
* Function:		SVGA8_divotSize
...
void _ASMAPI SVGA8_ditherPixel(int x,int y)
/****************************************************************************
*
* Function:		SVGA8_ditherPixel
