...
#include "mgldd.h"
#pragma hdrstop
#include "drivers\win\fdib24.h"

/*---------------------------- Driver Tables -----------------------------*/

drivertype	_VARAPI FULLDIB24_driver = {
	IDENT_FLAG,
	MGL_FULLDIB24NAME,
	MGL_FULLDIB24NAME,
	...
	grFULLDIB,
	FULLDIB24_detect,
	FULLDIB24_initDriver,
	};

/* List of mode specific device context information */

PRIVATE gmode_t FULLDIB24_modes[] = {
...
ibool _ASMAPI FULLDIB24_detect(int id,int force,int *driver,int *mode,
	modetab modes)
/****************************************************************************
*
* Function:		FULLDIB24_detect
...
****************************************************************************/
{
	if (force >= grFULLDIB && FULLDIB_detect()) {
		 ...
		 */
		FULLDIB_useMode(modes,grSVGA_640x480x16m,id,&FULLDIB24_modes[0]);
		FULLDIB_useMode(modes,grSVGA_800x600x16m,id,&FULLDIB24_modes[1]);
		FULLDIB_useMode(modes,grSVGA_1024x768x16m,id,&FULLDIB24_modes[2]);
		FULLDIB_useMode(modes,grSVGA_1152x864x16m,id,&FULLDIB24_modes[3]);
		FULLDIB_useMode(modes,grSVGA_1280x960x16m,id,&FULLDIB24_modes[4]);
		FULLDIB_useMode(modes,grSVGA_1280x1024x16m,id,&FULLDIB24_modes[5]);
		FULLDIB_useMode(modes,grSVGA_1600x1200x16m,id,&FULLDIB24_modes[6]);
		return true;
		}
	mode = mode;
	return false;
...
ibool _ASMAPI FULLDIB24_initDriver(MGLDC *dc,int driverId,int modeId,ulong hwnd,
	int virtualX,int virtualY,int numBuffers,ibool stereo,int refreshRate)
/****************************************************************************
*
* Function:		FULLDIB24_initDriver
* Parameters:	dc	- Device context.
...
****************************************************************************/
{
	...
	if (stereo)
    	return false;
	dc->mi = FULLDIB24_modes[modeId - grSVGA_640x480x16m];
	if (g_state == NULL) {
