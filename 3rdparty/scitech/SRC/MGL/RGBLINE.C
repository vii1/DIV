...
/****************************************************************************
DESCRIPTION:
Rasterize a smooth RGB shaded line.
...
REMARKS:
Draws an RGB shaded line in the current device context. The color values will be
linearly interpolated between the color at the first endpoint and the color at the second
endpoint, producing a smooth shaded line. Note that each RGB color channel is
interpolated individually, so this function will produce lines with properly blended RGB
colors.
****************************************************************************/
/* {secret} */
void ASMAPI MGL_rgbLineCoord(
	fix32_t x1,
	fix32_t y1,
	fix32_t r1,
	fix32_t g1,
	...
	fix32_t b2)
{
	...
	if (DC.a.clip && !_MGL_clipRGBLineFX(
			&x1,&y1,&r1,&g1,&b1,&x2,&y2,&r2,&g2,&b2,
			DC.clipRectFX.left,DC.clipRectFX.top,
			DC.clipRectFX.right,DC.clipRectFX.bottom))
		return;
	DC.r.rgbLine(
		x1 + DC.viewPortFX.left,y1 + DC.viewPortFX.top,r1,g1,b1,
		x2 + DC.viewPortFX.left,y2 + DC.viewPortFX.top,r2,g2,b2);
}
#endif	/* !MGL_FIX3D */
