...
#ifdef	MGL_FIX3D
/* {secret} */
ibool _MGL_clipRGBZLineFX(fix32_t *fx1,fix32_t *fy1,zfix32_t *fz1,
	fix32_t *fr1,fix32_t *fg1,fix32_t *fb1,
	fix32_t *fx2,fix32_t *fy2,zfix32_t *fz2,
	fix32_t *fr2,fix32_t *fg2,fix32_t *fb2,
	fix32_t left,fix32_t top,fix32_t right,fix32_t bottom)
/****************************************************************************
*
* Function:		_MGL_clipRGBZLineFX
* Parameters:	x1,y1,z1,c1	- First endpoint of line to clip
*				x2,y2,z2,c2	- Second endpoint of line to clip
...
****************************************************************************/
{
	...
	fix32_t		x,y,r,g,b,dx,dy,dz,dr,dg,db;
	zfix32_t	z;
	...
	while (true) {
		...
		if (outcodeOut & 1) {				/* Clip to bottom boundary	*/
			...
			r = r1 + MGL_FixMulDiv(dr,y,dy);
			g = g1 + MGL_FixMulDiv(dg,y,dy);
			b = b1 + MGL_FixMulDiv(db,y,dy);
			y = bottom-MGL_FIX_1;
			}
			...
			r = r1 + MGL_FixMulDiv(dr,x,dx);
			g = g1 + MGL_FixMulDiv(dg,x,dx);
			b = b1 + MGL_FixMulDiv(db,x,dx);
			x = right-MGL_FIX_1;
			}
		...
		if (outcodeOut == outcode1) {
			x1 = x;	y1 = y; z1 = z; r1 = r; g1 = g; b1 = b;
			SETOUTCODES(outcode1,x,y);
			}
		else {
			x2 = x; y2 = y; z2 = z; r2 = r; g2 = g; b2 = b;
