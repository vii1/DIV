...
int judas_set_s3m_pos(int pos)
{
  if(pos > header.ordernum-1 || pos < 0) return(0);

  if(pos > 0) s3m_pos=pos-1, s3m_wind =  1;
  else        s3m_pos=pos+1, s3m_wind = -1;
...
int judas_loads3m_mem(unsigned char *FileBuffer)
{
        int count;
        unsigned char chconverttable[S3M_MAXCHANNELS];
        INSTRUMENT *ip;
         ...
         */
        judas_error = JUDAS_READ_ERROR;
        memcpy(&header, FileBuffer+FilePos, sizeof header); FilePos+=sizeof header;
        /*
         * Check it is a S3M
         */
        judas_error = JUDAS_WRONG_FORMAT;
        ...
        memset(&s3m_order, 255, sizeof s3m_order);
        judas_error = JUDAS_READ_ERROR;
        memcpy(&s3m_order, FileBuffer+FilePos, header.ordernum); FilePos+=header.ordernum;
        judas_error = JUDAS_WRONG_FORMAT;
        {
          int firstpattern = S3M_MAXORDER+666;
          int firstloop = S3M_MAXORDER+666;
         ...
         */
        judas_error = JUDAS_READ_ERROR;
        memcpy(&instparapointers, FileBuffer+FilePos, header.instnum * sizeof(short)); FilePos+=(header.instnum * sizeof(short));
        memcpy(&pattparapointers, FileBuffer+FilePos, header.pattnum * sizeof(short)); FilePos+=(header.pattnum * sizeof(short));
        /*
         * Reserve intermediate buffer for S3M format instruments (to prevent
         * backwards seeks patterns & samples are loaded later)
         */
        ...
        if (header.panningmagic == 252)
        {
          memcpy(&defaultchannelpanpos, FileBuffer+FilePos, s3m_channels * sizeof(char)); FilePos+=(s3m_channels * sizeof(char));
          for (count = 0; count < s3m_channels; count++)
          {
            if (defaultchannelpanpos[count] & 32) {
              /* Channel panning pos defined */
        ...
        for (count = 0; count < header.instnum; count++)
        {
                 ...
                 */
                judas_error = JUDAS_READ_ERROR;
                FilePos=instparapointers[count] << 4;
                /*
                 * Read the instrument
                 */
                memcpy(s3mip, FileBuffer+FilePos, sizeof (S3M_INSTRUMENT)); FilePos+=sizeof (S3M_INSTRUMENT);
                if (s3mip->flags & 4)
                {
                        s3mip->length <<= 1;
                        s3mip->loopstart <<= 1;
        ...
        for (count = 0; count < header.pattnum; count++)
        {
                ...
                if (pattparapointers[count])
                {
                         ...
                         */
                        judas_error = JUDAS_READ_ERROR;
                        FilePos=pattparapointers[count] << 4;
                        memcpy(&loadpattlen, FileBuffer+FilePos, sizeof loadpattlen); FilePos+=sizeof loadpattlen;
                        ...
                        if (loadpattlen > 2)
                        {
                                ...
                                judas_error = JUDAS_READ_ERROR;
                                memcpy(packbuffer, FileBuffer+FilePos, loadpattlen); FilePos+=loadpattlen;
        ...
        for (count = 0; count < header.instnum; count++)
        {
                ...
                if ((s3mip->type == 1) && (s3mip->length))
                {
                         ...
                         * Seek & read sample
                         */
                        FilePos=(s3mip->paraptr | (s3mip->paraptrhigh << 16)) << 4;
                        memcpy(ip->sp->start, FileBuffer+FilePos, s3mip->length); FilePos+=s3mip->length;
                        /*
                         * Looped or non-looped?
                         */
                        if ((s3mip->flags & 1) && (s3mip->loopstart != s3mip->loopend))
