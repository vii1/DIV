...
int judas_set_xm_pos(int pos)
{
  if(pos > xm_header.songlength-1 || pos < 0) return(0);

  if(pos > 0) xm_pos=pos-1, xm_wind =  1;
  else        xm_pos=pos+1, xm_wind = -1;
...
int judas_loadxm_mem(unsigned char *FileBuffer)
{
        int count;
        int FilePos=0;
         ...
         */
        judas_error = JUDAS_WRONG_FORMAT;
        memcpy(&xm_id, FileBuffer+FilePos, sizeof xm_id); FilePos+=sizeof xm_id;
        /*
         * Check that it's an XM and the version is correct
         */
        if ((memcmp("Extended Module:", xm_id.id, 16)) || (xm_id.idbyte != 0x1a))
         ...
         */
        judas_error = JUDAS_READ_ERROR;
        memcpy(&xm_header, FileBuffer+FilePos, sizeof xm_header); FilePos+=sizeof xm_header;
        /*
         * Check that there aren't too many channels
         */
        if (xm_header.channels > CHANNELS)
         ...
         * If header is longer that what we use of it, skip the extra bytes
         */
        FilePos+=(xm_header.headersize - sizeof xm_header);
        /*
         * Load the patterns
         */
        for (count = 0; count < xm_header.patterns; count++)
        {
                 ...
                 */
                judas_error = JUDAS_READ_ERROR;
                memcpy(&load_pattern, FileBuffer+FilePos, sizeof load_pattern); FilePos+=sizeof load_pattern;
                /*
                 * Skip extra data in pattern header
                 */
                FilePos+=(load_pattern.headersize - sizeof load_pattern);
                /*
                 * Allocate memory for unpacked pattern & clear it
                 */
                pattern[count].rows = load_pattern.rows;
                ...
                if (load_pattern.packsize)
                {
                         ...
                         */
                        judas_error = JUDAS_READ_ERROR;
                        memcpy(pack_buffer, FileBuffer+FilePos, load_pattern.packsize); FilePos+=load_pattern.packsize;
                        packptr = pack_buffer;
                        unpackptr = pattern[count].data;
                        packleft = load_pattern.packsize;
                        while (packleft)
        ...
        for (count = 0; count < xm_header.instruments; count++)
        {
                 ...
                 */
                judas_error = JUDAS_READ_ERROR;
                memcpy(instr_ptr, FileBuffer+FilePos, 29); FilePos+=29;
                if (instr_ptr->numbersamples)
                {
                        int s;
                         ...
                         * There are samples; read the rest in
                         */
                        memcpy(((char *)instr_ptr) + 29, FileBuffer+FilePos, sizeof(XM_INSTRUMENT) - 29 - 650); FilePos+=(sizeof(XM_INSTRUMENT) - 29 - 650);
                        /*
                         * Skip over extra shit in instr. header
                         */
                        FilePos+=(instr_ptr->headersize - (sizeof(XM_INSTRUMENT) - 650));
                        /*
                         * Precalculate envelopes
                         */
                        if (instr_ptr->volenvflags & ENV_ON)
                        ...
                        for (s = 0; s < instr_ptr->numbersamples; s++)
                        {
                                 ...
                                 */
                                judas_error = JUDAS_READ_ERROR;
                                memcpy(xsp, FileBuffer+FilePos, sizeof(XM_SAMPLE)); FilePos+=sizeof(XM_SAMPLE);
                                /*
                                 * Hop over extra shit in sample header
                                 */
                                FilePos+=(instr_ptr->sampleheadersize - sizeof(XM_SAMPLE));
                                /*
                                 * If looplength is zero, loop must be de-
                                 * activated
                                 */
                        ...
                        for (s = 0; s < instr_ptr->numbersamples; s++)
                        {
                                ...
                                if (sp)
                                {
                                        ...
                                        judas_error = JUDAS_READ_ERROR;
                                        memcpy(sp->start, FileBuffer+FilePos, xsp->length); FilePos+=xsp->length;
                ...
                else
                {
                         ...
                         */
                        judas_error = JUDAS_READ_ERROR;
                        FilePos+=(instr_ptr->headersize - 29);
                }
        }
        judas_error = JUDAS_OK;
        xm_loaded = 1;
