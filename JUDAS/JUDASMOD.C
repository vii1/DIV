 ...
 */
#define HEU16_TO_UNSIGNED_SHORT(heu16) (((unsigned short) (heu16).high << 8) | (heu16).low)
...
int judas_set_mod_pos(int pos)
{
  if(pos > mt_length-1 || pos < 0) return(0);

  if(pos > 0) mt_pos=pos-1, mt_wind =  1;
  else        mt_pos=pos+1, mt_wind = -1;
...
int judas_loadmod(char *name)
{
        ...
        if (judas_read(handle, mod_patterns, mt_patternlength * mt_patterns) != mt_patternlength * mt_patterns)
        {
                judas_freemod();
                judas_close(handle);
                return 0;
        ...
        /* Now load instruments */
        {
                ...
                MOD_INSTRUMENT *mod_instr_ptr = (MOD_INSTRUMENT *)(&mod_info[MOD_INSTROFFSET]);
                ...
                for (count = 0; count < MOD_INSTRUMENTS; count++)
                {
                        ...
                        length = HEU16_TO_UNSIGNED_SHORT(mod_instr_ptr->length) << 1;
                        repeat = HEU16_TO_UNSIGNED_SHORT(mod_instr_ptr->repeat) << 1;
                        end = HEU16_TO_UNSIGNED_SHORT(mod_instr_ptr->replen) << 1;
...
int judas_loadmod_mem(unsigned char *FileBuffer)
{
        int count;
        NOTE *destptr;
        unsigned char *srcptr;
        ...
        judas_error = JUDAS_WRONG_FORMAT;
        /* Read the infoblock */
        memcpy(mod_info, FileBuffer+FilePos, MOD_INFOBLOCK); FilePos+=MOD_INFOBLOCK;
        /*
         * Clear byte 20 of header to prevent weird shit when printing
         * modulename
         */
        ...
        judas_error = JUDAS_READ_ERROR;
        memcpy(mod_patterns, FileBuffer+FilePos, mt_patternlength * mt_patterns); FilePos+=(mt_patternlength * mt_patterns);
        ...
        /* Now load instruments */
        {
                ...
                for (count = 0; count < MOD_INSTRUMENTS; count++)
                {
                        ...
                        if (length)
                        {
                                ...
                                memcpy(i_ptr->smp->start, FileBuffer+FilePos, length); FilePos+=length;
                                i_ptr->smp->repeat = i_ptr->smp->start + repeat;
                                i_ptr->smp->end = i_ptr->smp->start + end;
                                i_ptr->smp->voicemode = voicemode;
                                judas_ipcorrect(i_ptr->smp);
...
char *judas_getmodname(void)
{
        if (mod_info) return &mod_info[MOD_NAMEOFFSET];
        else return NULL;
}
...
static int init_modplayer(void)
{
        ...
        if (!judas_memlock(&modplayer_code_lock_start, (int)&modplayer_code_lock_end - (int)&modplayer_code_lock_start)) return 0;
        ...
        if (!judas_memlock(&mt_line, sizeof mt_line)) return 0;
        if (!judas_memlock(&mt_length, sizeof mt_length)) return 0;
        if (!judas_memlock(&mt_channels, sizeof mt_channels)) return 0;
        if (!judas_memlock(&mt_patterns, sizeof mt_patterns)) return 0;
        if (!judas_memlock(&mt_ticktempo, sizeof mt_ticktempo)) return 0;
        ...
        if (!judas_memlock(&mt_poshasbeenplayed, sizeof mt_poshasbeenplayed)) return 0;
        ...
        if (!judas_memlock(&mt_roundcounter, sizeof mt_roundcounter)) return 0;
        if (!judas_memlock(&mt_wind, sizeof mt_wind)) return 0;
        if (!judas_memlock(&mt_windingpause, sizeof mt_windingpause)) return 0;
...
void judas_stopmod(void)
{
        ...
        if (mt_channels > CHANNELS) mt_channels = CHANNELS;
        if (!mod_info) return;
        judas_player = NULL;
        for (count = 0; count < mt_channels; count++)
        {
        ...
        if ((!mt_tickcount) && (!mt_patterndelay))
        {
                ...
                for (count = mt_channels; count > 0; count--)
                {
                        ...
                        switch (tptr->effect)
                        {
                                ...
                                if (chptr->vol > 64*256) chptr->vol = 64*256;
                                ...
                                if (!tptr->effectdata)
                                {
                                  if (mt_roundcounter != 1)
                                  {
                                    ...
                                    if (mt_roundcounter) mt_roundcounter--;
                                    return;
                                  } else {
                                    judas_stopmod();
                                    return;
        ...
        if (mt_tickcount)
        {
                ...
                for (count = mt_channels; count > 0; count--)
                {
                        switch (tptr->effect)
                        {
                                ...
                                tptr->period = tptr->baseperiod + ((vibratotable[vibratotypetable[tptr->vibratotype & 3]][tptr->vibratophase] * tptr->vibratodepth) >> 5);
                                ...
                                chptr->vol = (tptr->vol + ((vibratotable[tptr->tremolotype & 3][tptr->tremolophase] * tptr->tremolodepth) >> 4)) * 256;
                                if (chptr->vol < 0) chptr->vol = 0;
                                if (chptr->vol > 64*256) chptr->vol = 64*256;
                                break;
        ...
        if (mt_tickcount >= mt_ticktempo)
        {
                ...
                if (!mt_patterndelay)
                {
                        ...
                        if (mt_pos != mt_oldpos)
                        {
                          ...
                          else mt_poshasbeenplayed[mt_pos] = 1;
                        }
                }
        }
        /*
